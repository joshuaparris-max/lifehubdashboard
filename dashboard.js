// File:// offline mode shim — keeps LifeHub usable when opened directly from the file system.
(function(){
  try {
...
const npcVisitors = [
  ...
  },
  {
    ...
    reward: {
      ...
      text: "Temporary badge: Ranger’s Crest (blue-green glow).",
    },
    icon: "🏹",
...
function grantXp(amount, reason) {
  ...
  if (actionFeedback) {
    actionFeedback.textContent = `+${amount} XP — ${reason || "reward"}`;
...
const LORE_BLURBS = {
  ...
  Work: "Guild contracts, forge permits, and apprenticeships—signed in wax and filed with care.",
  Family: "The hearth-ledger: healers’ notes, school scrolls, and guardians’ pledges.",
  Finance: "The guild treasury, watched by careful ledgers and stamped receipts.",
  Housing: "Keep walls, rental oaths, and inspection runes—proof the home stays defended.",
  Personal: "Identity seals, travel writs, and wellbeing sigils tuned to the bearer.",
  Projects: "Workbenches and prototypes—schematics humming with potential.",
  ...
  Templates: "Reusable incantations and scrolls—ready to clone for the next quest.",
...
const QUEST_POOL = [
  ...
  },
  {
    ...
    goal: "Touch Projects or Templates — update one doc.",
  ...
  },
  {
    ...
    goal: "Empty Downloads → Inbox or clear scratchpad.",
...
const TAVERN_BANTER = [
  "“The archivist swears the cleanest vault always finds the missing key.”",
  "A bard hums: “Three clicks to glory—refresh, review, rejoice.”",
  "“Empty the Downloads satchel nightly; gremlins fear a tidy pack.”",
  "The innkeeper whispers: “Back up your treasures; dragons love a second copy.”",
  "“Rumor says the map spell reveals more under a comet’s light.”",
  "A ranger notes: “Tag the trailhead—future-you thanks present-you.”",
  "“Quests done before noon grant double bragging rights.”",
  "The tinker: “Roll a d20 when stuck; high rolls mean ship it.”",
];
const ENCOUNTER_TABLE = [
  ...
  "Lost adventurer: rename a messy filename so it’s clear.",
...
const LOOT_TABLE = [
  ...
  "Scribe’s Quill: promote scratchpad to a file.",
  "Traveler’s Token: open a random card detail for inspiration.",
];
const ACHIEVEMENTS = [
  {
    id: "refresh",
...
const MINI_ADVENTURE_SETS = {
  clocktower: {
    ...
    nodes: {
      start: {
        text: "The clocktower’s bell will toll in 10 minutes. You and your crew stand at the rusted service door.",
        choices: [
      ...
      bluff: {
        text: "A sleepy guard opens a slit. “Passcode?”",
        choices: [
      ...
      force: {
        text: "The door groans open—noise echoes. You rush inside.",
      ...
      delivery: {
        text: "“Fine, leave it on the landing,” the guard mutters, unlatching the door.",
        choices: [{ label: "Thank him and head in", next: "stairs" }],
      },
      badge: {
        text: "He squints. “That badge is outdated.” Tension rises.",
        choices: [
      ...
      cut: {
        text: "The rope snaps—silence for now. The archive door is ahead.",
      ...
      chase: {
        text: "Alarms flare—the jam only bought seconds. You sprint down the stairs.",
  ...
  undersea: {
    ...
    nodes: {
      ...
      lamp: {
        text: "The lamp brightens—revealing a hidden trench downwards.",
  ...
  space: {
    ...
    nodes: {
      ...
      radio: {
        text: "Static... then a voice: “Identify yourself.”",
        choices: [
      ...
      inspect: {
        text: "The station hesitates. “Send clearance code.”",
      ...
      crate: {
        text: "You drift toward the crate—it’s tagged for a research bay.",
        choices: [{ label: "Open mid-space", next: "win" }],
      },
      ai: {
        text: "AI boots. “Captain, I can pilot us home.”",
  ...
  underground: {
    ...
    nodes: {
      start: {
        text: "You’re in a torch-lit tunnel, footsteps echoing behind.",
        choices: [
      ...
      weapon: {
        text: "The ore chunk glows faintly—reactive crystal.",
      ...
      win: {
        text: "Fresh air hits your face. You’re free—for now.",
        choices: [{ label: "Play again", next: "start" }],
      },
    },
  },
  desert: {
    ...
    nodes: {
      ...
      beach: {
        text: "Footprints—yours? A tide pool shimmers with shells.",
        choices: [
...
async function fetchJsonWithFallback(url, { cache = "default" } = {}) {
  ...
  if (window.location && window.location.protocol === "file:") {
    console.info(`Running in file:// mode — skipping fetch for ${url}`);
    return {};
  }
  try {
...
async function fetchTextWithFallback(url, { cache = "default" } = {}) {
  ...
  if (window.location && window.location.protocol === "file:") {
    console.info(`Running in file:// mode — skipping text fetch for ${url}`);
    return "";
  }
  try {
    const response = await fetch(url, { cache });
...
const PANEL_ICON_MAP = {
  "copilot-panel": "🔍",
  "automation-scheduler-panel": "🛠️",
  "health-panel": "💠",
  "timeline-panel": "🧭",
  "recent-files-panel": "🗂️",
  "wellbeing-widget": "💜",
  "card-grid": "📌",
  "resource-spotlight": "🔦",
  playables: "🎮",
...
const STAT_ICON_MAP = {
  "Inbox items awaiting filing": "📬",
  "Family health records": "🩺",
  "Finance docs": "💰",
  "Active projects (code + docs)": "🧱",
  "Housing & property files": "🏠",
  "Media assets": "🎞️",
  "Archive entries": "🗄️",
  "Templates ready": "📄",
};
...
function renderQuestBoard() {
  ...
  questStreak.textContent = `Streak: ${questState.streak || 0}${questState.completed ? " ✓" : ""}`;
...
function renderTavernLine() {
  ...
  if (!TAVERN_BANTER.length) {
    tavernLine.textContent = "No tavern tales yet—add some to TAVERN_BANTER.";
...
function rollEncounter() {
  ...
  logDiceResult(`Encounter (d${ENCOUNTER_TABLE.length}→${roll}): ${entry}`);
}

function rollLoot() {
  const roll = rollDie(LOOT_TABLE.length);
  const entry = LOOT_TABLE[roll - 1] || "Empty satchel.";
  logDiceResult(`Loot (d${LOOT_TABLE.length}→${roll}): ${entry}`);
}
...
function rollDndCheck({ mode = "normal" } = {}) {
  ...
  pieces.push(`= ${total}`, "→", success ? "SUCCESS" : "fail", `vs DC ${dc}`);
  logDndResult(pieces.join(" "));
  if (inspirationUsed) {
    showToast({ icon: "✨", label: "Inspiration used", meta: `Rolls ${rolls.join(" / ")}` });
  }
  if (bardicUsed) {
    showToast({ icon: "🎶", label: `Bardic +${bardicBonus}`, meta: `Total ${total}` });
  }
}
...
function setupMiniAdventureSelector() {
  ...
  select.innerHTML = Object.entries(MINI_ADVENTURE_SETS)
    ...
    .join("") + `<option value="random">🎲 Roll random</option>`;
...
function getCharacterContext() {
  ...
  return {
    ...
    hasAttuned,
    inventory: inv,
    flavor: flavorParts.filter(Boolean).join(" · "),
  };
}
...
function renderMiniNode(key) {
  ...
  miniText.textContent = node.text || "—";
  if (context.flavor) {
...
function applyMiniConsequences(result) {
  ...
  if (result.success) {
    ...
    setActionFeedback(`Success! +6 XP${eased ? " · tension eased (-1 penalty)" : ""}`);
  } else {
...
function renderMiniRollResult(result) {
  ...
  rollBox.textContent = `${label || "Check"}: d20 ${roll} ${modText}${penaltyText} = ${total} vs DC ${dc} → ${success ? "PASS" : "FAIL"}`;
}
...
function renderCharacterCard(level = 1) {
  ...
  notesEl && (notesEl.textContent = notes ? `Background: ${notes}` : 'Background: —');
...
function awardXp(amount = 0, reason = "") {
  ...
  saveXpState(xpState);
  renderXpBadge();
  if (reason) setActionFeedback(`+${delta} XP — ${reason}`);
}
    ...
    return parsed
      .map((item) => {
        ...
        return {
          ...
          label,
          url,
          icon: (item.icon || "").toString().trim().slice(0, 3) || "⚡",
...
function handleCustomQuickAdd() {
  ...
  const icon = (customQuickIconInput?.value || "").trim().slice(0, 3) || "⚡";
...
function showToast({ icon = '🎉', label = 'Nice!', meta = '' } = {}) {
...
function setGameCommandBusy(variant = "base", busy = false) {
  ...
  if (button) {
    ...
    button.textContent = busy ? "Running…" : "Send";
  }
...
function renderSlotMeta(variant = "base") {
  ...
  statusEl.textContent = `Slot ${slot.toUpperCase()} · saved ${timeLabel}`;
...
if (npcGroqKeyInput && !npcGroqKeyInput.value && typeof window !== "undefined" && window.LIFEHUB_GROQ_KEY) {
  ...
  if (inboxStat) {
    const observer = new MutationObserver(() => {
      ...
      if (val === 0) {
        showToast({ icon: "📬", label: "Inbox zero!", meta: "Nice sweep." });
      }
    });
...
async function loadRecentFiles({ force = false } = {}) {
  ...
  showMetaMessage(recentGrid, 'Loading recent files…');
  ...
  if (isFileProtocol) {
    ...
    showMetaMessage(recentGrid, 'Recent file snapshot missing — run scripts/refresh_all.sh to rebuild dashboard-inline-data.js.');
    return;
  }
  try {
...
function applyRecentFilesPayload(payload) {
  if (!payload) {
  showMetaMessage(recentGrid, 'Recent file list unavailable — run generate_recent_files.py.');
    return;
  }
    ...
    xhr.onload = () => {
      if (xhr.status === 0 || (xhr.status >= 200 && xhr.status < 300)) {
        try {
          ...
          console.error(error);
          showMetaMessage(recentGrid, 'Recent files JSON unreadable — rerun generate_recent_files.py.');
        }
      } else {
  showMetaMessage(recentGrid, 'Recent files unavailable — serve dashboard via http://localhost.');
      }
    };
    xhr.onerror = () => {
  showMetaMessage(recentGrid, 'Recent files unavailable — serve dashboard via http://localhost.');
...
function renderDownloadsFeed(payload) {
  ...
  staleFiles
    ...
    .forEach((file) => {
      ...
      const small = document.createElement('small');
      small.textContent = `${age}h old · ${file.modified ? formatDate(file.modified) : 'Unknown date'}`;
      li.appendChild(strong);
      li.appendChild(small);
      fragment.appendChild(li);
    });
...
function renderResurfaceCard() {
  ...
  if (!filtered.length) {
    showMetaMessage(resurfaceCard, 'All clear — add more files to LifeHub to see resurfaced suggestions.');
    return;
  }
...
function renderBackupStatus() {
  ...
  if (backupSummary) {
    backupSummary.textContent = `${computed.length} targets · ${dangerCount} overdue · ${warningCount} due soon · ${okCount} healthy`;
